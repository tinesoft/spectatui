...
- ~~Catalog search/browse of not-yet-installed extensions, presets, and workflows... intentionally out of scope~~ — **delivered** (see 2026-07-14 refinement note): the manager views' existing inline `/` filter also narrows each catalog's "available" (not-yet-installed) rows, so browsing is now covered by the same lifecycle-action manager views rather than a separate discovery surface. Self-check/self-upgrade of the underlying CLI tool remains out of scope for this initial version.
- Catalog "available" items (not-yet-installed extensions, presets, workflows, integrations) are discovered by fetching each configured catalog source's JSON directly, rather than by shelling out to `specify <target> search`/`list --available` and parsing its Rich-formatted table output — chosen because that output can't be efficiently re-queried per keystroke (for the inline filter) or bound to a stable per-row install action, unlike a structured JSON record. Every *mutating* action (install, remove, enable/disable, reprioritize, switch, run) still exclusively goes through a real `specify` subprocess call; this is a read/discovery-only exception to the CLI-mediated principle (see `design/core/spectatui-archi-design.md` §1.5).
