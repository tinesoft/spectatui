...
pub fn draw(frame: &mut Frame, app: &App) {
    let theme = &app.theme;
    let workflows = &app.project.workflows;

    let full = frame.area();
    let w = full.width.saturating_sub(6).clamp(40, 100);
    let h = (workflows.len() as u16 + 6)
    ...
    let title = Line::from(vec![
        ...
        Span::styled("Automation Workflows · specify workflow", theme.accent_bold),
        Span::styled(" ├", theme.border_focused),
    ]);
    ...
    if area.height > 2 {
        ...
        frame.render_widget(
            Paragraph::new(Line::from(Span::styled(
                "run executes the full lifecycle as a tracked job · esc close",
                theme.faint_style,
            ))),
            footer_area,
        );
